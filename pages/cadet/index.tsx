    ...
    <main style={{ padding: 24, fontFamily: "system-ui" }}>
        ...
        <section style={{ marginTop: 12, border: "1px solid #ddd", padding: 12, borderRadius: 10 }}>
          ...
          {dash.submission ? (
            dash.submission.score == null ? (
              <p>
                <b>Submitted.</b> Waiting for instructor grade.
              </p>
            ) : (
              <>
                <p>
                  Score: <b>{dash.submission.score}</b>/100
                </p>
                ...
                {dash.submission.instructorNotes ? (
                  ...
                  <p style={{ color: "#666" }}>(No instructor notes)</p>
                )}
              </>
            )
          ) : (
            <p style={{ color: "#666" }}>
              No submission yet for attempt <b>{dash?.attempt ?? 1}</b>.
            </p>
          )}
        </section>
      )}
      ...
      <section style={{ marginTop: 16 }}>
        {!dash ? (
          ...
          <>
            <p>
              Team: <b>{dash.team.name}</b>
            </p>
            <p>
              Attempt: <b>{dash.attempt}</b>
            </p>

            {dash.submission && dash.submission.score == null ? (
              <p style={{ color: "#a00" }}>Tryout is locked while waiting for instructor grading.</p>
            ) : (
