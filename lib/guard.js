const { prisma } = require("./prisma");
const { audit } = require("./audit");

async function requireCadetActive(userId) {
  const user = await prisma.user.findUnique({ where: { id: userId } });
  if (!user) throw new Error("UNAUTHORIZED");
  if (user.lockedReason) throw new Error("LOCKED");

  const membership = await prisma.teamMember.findFirst({
    where: { userId },
    orderBy: { teamId: "asc" }, // deterministic
    include: { team: { include: { session: true, notes: true } } }
  });  
  if (!membership) throw new Error("NO_TEAM");

  const team = membership.team;
  const session = team.session;

  if (!session || session.status === "NOT_STARTED") {
    return { team, session: session || null, state: "NOT_STARTED" };
  }

  if (session.status === "ACTIVE") {
    const now = new Date();

    // ---- Expiration check ----
    if (session.endsAt && now > session.endsAt) {
      await prisma.teamSession.update({
        where: { teamId: team.id },
        data: { status: "EXPIRED" }
      });

      const members = await prisma.teamMember.findMany({
        where: { teamId: team.id }
      });
      const ids = members.map((m) => m.userId);

      await prisma.user.updateMany({
        where: { id: { in: ids } },
        data: {
          lockedReason:
            "Tryout expired (4 hours elapsed). Instructor unlock required."
        }
      });

      await audit({
        teamId: team.id,
        userId,
        type: "TRYOUT_EXPIRED_AUTOLOCK"
      });

      throw new Error("EXPIRED_LOCKED");
    }

    // ---- Submission lockout check ----
    const attemptNumber = session.attemptNumber || 1;

    // ---- AccessGrant check (intel split) ----
const grant = await prisma.accessGrant.findUnique({
  where: {
    teamId_attemptNumber_userId: {
      teamId: team.id,
      attemptNumber,
      userId
    }
  }
});

if (!grant) {
  // This means /api/cadet/start did not assign grants successfully for this attempt.
  throw new Error("NO_ACCESS_GRANT");
}


    const existingSubmission = await prisma.submission.findFirst({
      where: {
        teamId: team.id,
        userId,
        attemptNumber
      },
      orderBy: { createdAt: "desc" }
    });

    if (existingSubmission && existingSubmission.score == null) {
      return {
        team,
        session,
        state: "SUBMITTED_WAITING_GRADE",
        grant
      };
    }    

    return { team, session, state: "ACTIVE", grant };
  }

  if (session.status === "EXPIRED") {
    throw new Error("EXPIRED_LOCKED");
  }

  throw new Error("UNKNOWN_STATE");
}

module.exports = { requireCadetActive };
